...
import { Textarea } from '@/components/ui/textarea'; // Textarea untuk Edit Ringkasan
import { DocCategorySelect } from '@/components/DocCategorySelect';
import { toast } from 'sonner';
import { Calendar, FileText, User, Tag, Edit2, ExternalLink, Loader2, Save, X, Copy, Check, Clock, ShieldCheck, Info } from 'lucide-react';
...
export function DocumentDetailModal({ documentId, isOpen, onClose }: DocumentDetailModalProps) {
  ...
  const handleOpenEdit = () => {
    ...
    setEditForm({
      ...
      description: doc.description || '', // Tambahan untuk edit
    });
    setMode('edit');
  };
              ...
              {mode === 'edit' ? (
                <div className="p-6 max-w-4xl mx-auto space-y-6 pb-20">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1"><Label>Klasifikasi</Label><Input value={editForm.classification} onChange={e => setEditForm({ ...editForm, classification: e.target.value })} /></div>
                    <div className="space-y-1 md:col-span-2">
                      <Label>Ringkasan (Description)</Label>
                      <Textarea 
                        value={editForm.description} 
                        onChange={e => setEditForm({ ...editForm, description: e.target.value })} 
                        rows={4}
                        placeholder="Masukkan ringkasan singkat mengenai surat ini..."
                      />
                    </div>
                  </div>
                </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="pt-4 mt-2">
                        <p className="text-sm text-muted-foreground mb-2 flex items-center gap-2">
                          <Info className="w-4 h-4"/> Ringkasan Surat
                        </p>
                        <div className="p-3 rounded-lg bg-background border border-border text-sm leading-relaxed">
                          {doc.description || <span className="text-muted-foreground italic">Tidak ada ringkasan tersedia.</span>}
                        </div>
                      </div>

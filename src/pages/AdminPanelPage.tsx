...
import { UserCog, Shield, Eye, Users, Search, Edit2, UserPlus, KeyRound, ArrowUp, ArrowDown, ArrowUpDown, Loader2 } from 'lucide-react';
...
export default function AdminPanelPage() {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      // PERBAIKAN: Menyertakan 'position' (Jabatan) ke dalam payload invoke
      const { data, error } = await supabase.functions.invoke('create-user', {
        body: { 
          ...
          nip: newUser.nip, 
          role: newUser.role,
          position: newUser.position // Menambahkan jabatan
        },
        headers: {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    return result.sort((a, b) => {
      const valA = (a[sortConfig.key as keyof typeof a] || '').toString().toLowerCase();
      const valB = (b[sortConfig.key as keyof typeof b] || '').toString().toLowerCase();
      
      if (valA < valB) return sortConfig.direction === 'asc' ? -1 : 1;
      if (valA > valB) return sortConfig.direction === 'asc' ? 1 : -1;
      return 0;
    });
  }, [employees, searchQuery, filterRole, sortConfig]);
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50">
                  {renderSortableHeader('NIP', 'nip')}
                  {renderSortableHeader('Nama', 'name')}
                  {renderSortableHeader('Email', 'email')}
                  {renderSortableHeader('Jabatan', 'position')}
                  ...
                  filteredAndSorted.map((emp, i) => {
                    ...
                    return (
                      <TableRow key={emp.id} className={i % 2 === 0 ? '' : 'bg-muted/20'}>
                        <TableCell className="text-sm font-mono font-medium">{emp.nip}</TableCell>
